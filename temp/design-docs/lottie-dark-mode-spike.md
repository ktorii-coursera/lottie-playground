...
| **Risk** | ✅ No risk — proven workflow, no tooling dependency. | ◐ Low risk — dependent on LottieCreator's capabilities and roadmap, but no custom tooling to maintain. | ❌ High risk — if animation needs go beyond what the script supports, the engineering investment in custom tooling (Figma plugin, AE plugin, conversion script) becomes wasted effort and complicates the workflow. | ✅ Low risk — the plugin is additive. If it can't handle something, the animator does it manually. Worst case, it's Approach 1 with some automation. Also serves as a low-commitment way to validate whether a full pipeline (Approach 3) is worth investing in. |
| | | | | |
