...
Each approach is shown with its initial workflow and iteration loop (what happens when the animator needs to make a change). Each POC uses the same source animation to enable direct comparison.
...
Tokens: Manual selection from Figma design system
Plugins: None
Code: Theme-aware file loader
```

**Iteration (e.g., adding a new element):**
...
### Approach 2: LottieCreator Theming
...
### Approach 4: AE Auto-Apply Plugin
...
| Color transition (keyframe from color A to color B) | ✅ | TBD — needs POC testing | TBD — needs POC testing | ◐ Plugin attempts, animator cleans up |
